 ...
 */
var panel = new Polyer.Panel(true);
...
points.createProcessingLine(src, [{type: "gray"}, {type: "blur"}, {type: "edge",kernelSize:5}], panel.vertexNum, function () {
    points.createPolyMesh(convertHexToRGB(panel.meshColor));
//        panel.maxVertexNum = points.getEdgetPoins().length;
    o.emit("points");
});
poly.createOriginal(src, function () {
    o.emit("poly")
})
...
panel.onVertexNumChange = function (num) {
    ...
    if (points.getEdgetPoins().length == 0) {
        return;
    }
 ...
 * */
Polyer.Utils.bind(canvas_box, "mousewheel", Polyer.Utils.debounce(function (e) {
    if (e.wheelDelta > 0) {
        zoomLarger()
    } else {
        zoomSmaller()
